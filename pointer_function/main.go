...
func changeProductByPointer(p *Product) {
	(*p).price = 300 //equivalent to p.price
	p.productName = "Bicycle"
...
func main() {
	...
	change(p)
	fmt.Println("value of p after calling change(): ", x)

	fmt.Println("Value of x before calling changeVar(): ", x)
	changeVar(x)
	fmt.Println("value of x after calling changeVar(): ", x) //Will be unchanged
